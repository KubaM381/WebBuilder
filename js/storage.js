...
(() => {
  ...
  function normalizeRuntimeState() {
    const products = window.WebBuilderProducts, cart = window.WebBuilderCart, headerFooter = window.WebBuilderHeaderFooter, canvas = window.WebBuilderCanvas;
    if (products?.normalizeState) products.normalizeState(); if (cart?.normalizeState) cart.normalizeState(); if (headerFooter?.normalizeState) headerFooter.normalizeState(); if (canvas?.normalizeState) canvas.normalizeState(); return state;
  }
  function createSnapshot(source = state) { return clone({ elements: source.elements || [], cartItems: source.cartItems || [], cartButtonLabel: source.cartButtonLabel || "Zur Kasse gehen", cartConfig: source.cartConfig || {}, products: source.products || [], canvasHeight: source.canvasHeight || 1100, zoomLevel: source.zoomLevel || 0.85, appliedDiscountPercent: source.appliedDiscountPercent || 0, appliedDiscountLabel: source.appliedDiscountLabel || "", headerEnabled: !!source.headerEnabled, headerSticky: !!source.headerSticky, headerHeight: source.headerHeight || 64, headerBgColor: source.headerBgColor || "#111827", headerItems: source.headerItems || [], footerEnabled: !!source.footerEnabled, footerHeight: source.footerHeight || 70, footerBgColor: source.footerBgColor || "#111827", footerItems: source.footerItems || [], background: source.background || null }); }
