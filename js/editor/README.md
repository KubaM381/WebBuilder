...
| `sections-inspector.js` | Right-hand panel for the Phase 2 flow-layout selection (Section/Row/Card) — `#section-inspector-form`, parallel to `#bar-inspector-form`/`#cart-inspector-form`. Reads/writes via `window.WebBuilderSections` (`canvas/sections-data.js`) and reacts to `canvas/sections-render.js`'s selection through `state.selectedSectionRef`. No load-order requirement — reaches both only through their public APIs at runtime. | `window.WebBuilderSectionsInspector` |
