...
(() => {
  ...
  // "settings" ergänzt: web.html bietet dieses Icon in der Palette an
  // (data-icon="settings"); ohne Registry-Eintrag rendert canvas.js
  // gezogene Instanzen fälschlich als Text-Platzhalter statt als Icon.
  ...
  const register = (name, markup) => { if (!name || !markup) return false; icons[name] = String(markup); return true; };
  const getIcon = name => icons[name] || null;
  const getAllIcons = () => Object.assign({}, icons);
  // NEU (Offene Punkte #5 "Eigene Icons"): erlaubt es, ein zusätzliches
  // Icon zur Laufzeit per SVG-Code oder Bild-URL zu registrieren. Die
  // eigentliche Palette-UI (Formular + Kacheln + Drag&Drop) lebt in
  // canvas.js, da sie auf dessen bestehender Palette-Drag&Drop-Logik
  // aufbaut (siehe README, Projektregel 12: keine Duplikation). Diese
  // Registry hier bleibt bewusst reine Daten-Logik ohne DOM-Zugriff.
  //
  // Eigene Icons werden absichtlich NICHT persistiert (nicht Teil von
  // state.js/storage.js) — wie im ursprünglichen (vor-modularen) Builder
  // gelten sie nur für die aktuelle Sitzung. Siehe README "Offene Punkte".
  ...
  function getCustomIconNames() { return customIconOrder.slice(); }
  ...
  window.WebBuilderIconRegistry = { register, get: getIcon, getAll: getAllIcons, addCustom: addCustomIcon, getCustomNames: getCustomIconNames };
})();
