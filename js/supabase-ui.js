...
// js/supabase-data.js (per ES import, siehe dortiger Kommentar zur
// Begründung "import statt window.WebBuilderXxx").
import {
...
function bindModalView() {
  ...
  document.querySelectorAll(".cloud-page-delete").forEach(btn => btn.addEventListener("click", async e => {
    ...
    if (error) return;
    const ref = loadRef();
    if (ref?.pageId === id) saveRefIfPossible({ projectId: ref.projectId, pageId: null });
    await openCloudModal();
  }));
