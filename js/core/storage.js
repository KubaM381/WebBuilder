...
(() => {
  ...
  function normalizeRuntimeState() {
    const elements = window.WebBuilderElements, products = window.WebBuilderProducts, cart = window.WebBuilderCart, headerFooter = window.WebBuilderHeaderFooter, canvas = window.WebBuilderCanvas, cartPreviewBars = window.WebBuilderCartPreviewBars, sections = window.WebBuilderSections;
    if (elements?.normalizeState) elements.normalizeState(); if (products?.normalizeState) products.normalizeState(); if (cart?.normalizeState) cart.normalizeState(); if (headerFooter?.normalizeState) headerFooter.normalizeState(); if (canvas?.normalizeState) canvas.normalizeState(); if (cartPreviewBars?.normalizeState) cartPreviewBars.normalizeState(); if (sections?.normalizeState) sections.normalizeState(); return state;
  }
  // NOTE: cartPreviewHeader*/cartPreviewFooter* are top-level state.*
  // fields (not nested under an existing object like cartConfig), so per
  // root README.md rule 4 every one of them must be listed explicitly
  // here or it is silently lost on save/undo/reload. schemaVersion/
  // sections (Phase 2, see canvas/sections-data.js) follow the same rule.
  // The advanced styling fields (BgType/BgImage/TextColor/FontFamily/
  // Bold/Icon/IconImage) follow the same "add default here too" rule as
  // any other new top-level field — see js/shop/cart-preview-bars.js
  // normalizeState().
  function createSnapshot(source = state) { return clone({ schemaVersion: source.schemaVersion || 2, sections: Array.isArray(source.sections) ? source.sections : [], elements: source.elements || [], cartItems: source.cartItems || [], cartButtonLabel: source.cartButtonLabel || "Zur Kasse gehen", cartConfig: source.cartConfig || {}, products: source.products || [], canvasHeight: source.canvasHeight || 1100, zoomLevel: source.zoomLevel || 0.85, appliedDiscountPercent: source.appliedDiscountPercent || 0, appliedDiscountLabel: source.appliedDiscountLabel || "", headerEnabled: !!source.headerEnabled, headerSticky: !!source.headerSticky, headerHeight: source.headerHeight || 64, headerBgType: source.headerBgType || "solid", headerBgColor: source.headerBgColor || "#111827", headerBgImage: source.headerBgImage || "", headerItems: source.headerItems || [], footerEnabled: !!source.footerEnabled, footerHeight: source.footerHeight || 70, footerBgType: source.footerBgType || "solid", footerBgColor: source.footerBgColor || "#111827", footerBgImage: source.footerBgImage || "", footerItems: source.footerItems || [], cartPreviewHeaderEnabled: !!source.cartPreviewHeaderEnabled, cartPreviewHeaderHeight: source.cartPreviewHeaderHeight || 64, cartPreviewHeaderColor: source.cartPreviewHeaderColor || "#111827", cartPreviewHeaderLabel: source.cartPreviewHeaderLabel || "Header", cartPreviewHeaderBgType: source.cartPreviewHeaderBgType || "solid", cartPreviewHeaderBgImage: source.cartPreviewHeaderBgImage || "", cartPreviewHeaderTextColor: source.cartPreviewHeaderTextColor || "#ffffff", cartPreviewHeaderFontFamily: source.cartPreviewHeaderFontFamily || "inherit", cartPreviewHeaderBold: !!source.cartPreviewHeaderBold, cartPreviewHeaderIcon: source.cartPreviewHeaderIcon || null, cartPreviewHeaderIconImage: source.cartPreviewHeaderIconImage || "", cartPreviewFooterEnabled: !!source.cartPreviewFooterEnabled, cartPreviewFooterHeight: source.cartPreviewFooterHeight || 70, cartPreviewFooterColor: source.cartPreviewFooterColor || "#111827", cartPreviewFooterLabel: source.cartPreviewFooterLabel || "Footer", cartPreviewFooterBgType: source.cartPreviewFooterBgType || "solid", cartPreviewFooterBgImage: source.cartPreviewFooterBgImage || "", cartPreviewFooterTextColor: source.cartPreviewFooterTextColor || "#ffffff", cartPreviewFooterFontFamily: source.cartPreviewFooterFontFamily || "inherit", cartPreviewFooterBold: !!source.cartPreviewFooterBold, cartPreviewFooterIcon: source.cartPreviewFooterIcon || null, cartPreviewFooterIconImage: source.cartPreviewFooterIconImage || "", background: source.background || null }); }
