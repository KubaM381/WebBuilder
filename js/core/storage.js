...
(() => {
  ...
  function normalizeRuntimeState() {
    const elements = window.WebBuilderElements, products = window.WebBuilderProducts, cart = window.WebBuilderCart, headerFooter = window.WebBuilderHeaderFooter, canvas = window.WebBuilderCanvas, cartPreviewBars = window.WebBuilderCartPreviewBars;
    if (elements?.normalizeState) elements.normalizeState(); if (products?.normalizeState) products.normalizeState(); if (cart?.normalizeState) cart.normalizeState(); if (headerFooter?.normalizeState) headerFooter.normalizeState(); if (canvas?.normalizeState) canvas.normalizeState(); if (cartPreviewBars?.normalizeState) cartPreviewBars.normalizeState(); return state;
  }
  ...
  // here or it is silently lost on save/undo/reload. The advanced
  // styling fields (BgType/BgImage/TextColor/FontFamily/Bold/Icon/
  // IconImage) follow the same "add default here too" rule as any other
  // new top-level field — see js/shop/cart-preview-bars.js normalizeState().
