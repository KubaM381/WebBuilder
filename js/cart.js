...
(() => {
  ...
  // FIX (README Offener Punkt #4): `compareAtPrice` war ein totes Datenfeld
  // ohne jede UI (weder Produkt-Editor noch Warenkorb-Zeile zeigten es je
  // an) — die tatsächliche Rabattlogik läuft vollständig über
  // `discountPrice`. Entfernt statt mit ungenutzter Doppel-Logik zu leben
  // (siehe Projektregel 23 "keine unnötige Komplexität"). Bereits im
  // localStorage/Supabase gespeicherte alte Projekte mit `compareAtPrice`
  // laden weiterhin problemlos — das Feld wird beim Normalisieren einfach
  // nicht mehr übernommen.
  function normalizeProduct(product = {}) { const price = Number(product.price) || 0; const discountPrice = product.discountPrice != null && product.discountPrice !== "" ? Number(product.discountPrice) || 0 : null; return { id: product.id || `prod_${Date.now()}_${Math.random().toString(36).slice(2,8)}`, name: product.name || "Neues Produkt", price, discountPrice: discountPrice != null && discountPrice > 0 && discountPrice < price ? discountPrice : null, icon: product.icon || "📦", description: product.description || "" }; }
