...
(() => {
  ...
  function apply(mode = state.isPreviewMode) {
    ...
    const b = document.getElementById("btn-mode-toggle");
    if (b) b.innerHTML = state.isPreviewMode ? "✏️ Editor-Modus" : "👁️ Vorschau";
    window.WebBuilderCanvas?.applyZoom(state.isPreviewMode);
  ...
  function bindToggle() {
    const b = document.getElementById("btn-mode-toggle");
    if (!b || b.dataset.webBuilderPreviewBound === "true") return;
    b.dataset.webBuilderPreviewBound = "true";
  ...
  function scrollToTop() {
    return performScroll(() => 0);
  }

  function scrollToBottom() {
    return performScroll(container => container.scrollHeight);
  }
  ...
  function getActionType(item = {}) {
    return item.actionType || "none";
  }

  function execute(item = {}) {
    const type = getActionType(item);
    const url = item.actionUrl || "";
    const message = item.actionMsg || "Aktion ausgeführt!";
    switch (type) {
      case "scroll-top":
        return scrollToTop();
      case "scroll-bottom":
      ...
      case "open-url":
        ...
        return true;
      case "cart-add": {
        const product = window.WebBuilderProducts?.getById?.(item.productId);
        if (!product || !window.WebBuilderCart) return false;
        window.WebBuilderCart.addItem(product);
        return true;
      }
      case "open-cart-drawer":
        return !!window.WebBuilderCartRuntime?.open?.();
      case "open-custom-modal":
        return !!window.WebBuilderModals?.open?.(item.modalTitle || "Information", item.modalBody || message, item.modalFooter || "");
      // item.messagePosition picks the on-screen corner via openPositionedMessage().
      case "alert-msg":
        if (window.WebBuilderModals?.openPositionedMessage) {
          window.WebBuilderModals.openPositionedMessage(message, item.messagePosition || "bottom-right");
        } else if (window.WebBuilderModals?.openMessage) {
          window.WebBuilderModals.openMessage(item.modalTitle || "Hinweis", message);
        } else {
